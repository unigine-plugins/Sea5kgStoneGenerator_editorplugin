...
namespace FileSystem {
	...
	void saveRuntimeMeta(string asset, string runtime, string alias, int primary) {
		Json json = new Json();
		
		addKnownFile(asset);
		UGUID guid = getGUID(asset);
		
		json.addChild("version", ENGINE_VERSION);
		json.addChild("asset", guid.getString());
		json.addChild("alias", alias);
		json.addChild("primary", primary);
		
		add_dir(runtime);
		
		string path = runtime + ".meta";
		
		if(!json.save(path))
			Log::error("FileSystem::saveRuntimeMeta failed to save \"%s\"", path);
	}
	
	void saveRuntimeMetaTexture(string asset, string runtime) {
		string alias = setExtension(basename(asset), ".dds");
		saveRuntimeMeta(asset, runtime, alias, 1);
	}
	
	string getRuntimesFile(string path) {
		addKnownFile(path);
		
		UGUID guid = getGUID(path);
		string guid_str = guid.getString();
		return data_path + ".runtimes/" + substr(guid_str, 0, 2) + "/" + guid_str + "." + extension(path);
	}
