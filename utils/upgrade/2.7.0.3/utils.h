...
int getFilesWithExt(string files[],string path,string pattern) {
	return getFiles(files,path,[](string f) { return matchExtension(lower(getExtension(f)),pattern); });
}
...
int findDirectories(string directories[],string path) {
	
	int _walk_directory(string directories[],string path) {
		...
		forloop(int i = 0; dir.getNumDirs()) {
			...
			if(!_walk_directory(directories,d)) {
				dir.close();
				delete dir;
				return false;
			}
	...
	directories.clear();
	return _walk_directory(directories, path);
}
