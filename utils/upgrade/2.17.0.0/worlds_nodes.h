...
namespace WorldsNodes {
	...
	void processFile(string name) {
		...
		if(pathExtensionCompare(name, ".node")) {
			...
			int is_node_layer = xml.getNumChildren() > 1;
	...
	// modified!
	void processNodes(Xml xml, Xml xml_base) {
		
		Process::node(xml, xml_base);
