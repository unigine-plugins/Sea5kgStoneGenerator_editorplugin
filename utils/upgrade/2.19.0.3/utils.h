...
void remove_directory(string directory) {
	rmdir(directory, 1);
	Log::info(" Remove directory %s", directory);
}
...
int xmlRenameChild(Xml xml, string from, string to) {
...
Xml xmlAddChild(Xml xml, string name) {
	int num = xml.findChild(name);
	if (num != -1)
		return xml.getChild(num);
...
void xmlRenameArg(Xml xml, string from, string to) {
	int num = xml.findArg(from);
	if(num != -1)
		xml.setArgName(num, to);
}
...
//
string jsonGetString(Json json, string name, string default_value = "")
{
	Json child = json.getChild(name);
	if (child != NULL)
		return child.getString();
	return default_value;
}
void jsonSetString(Json json, string name, string value)
{
	Json child = json.getChild(name);
	if (child != NULL)
		child.setString(value);
	else
		json.addChild(name).setString(value);
}

string jsonGetInt(Json json, string name, int default_value = 0)
{
	Json child = json.getChild(name);
	if (child != NULL)
		return child.getInt();
...
void jsonSetInt(Json json, string name, int value)
{
	Json child = json.getChild(name);
	if (child != NULL)
		child.setInt(value);
	else
		json.addChild(name).setInt(value);
}
void jsonSetBool(Json json, string name, int value)
{
	Json child = json.getChild(name);
	if (child != NULL)
		child.setBool(value);
	else
		json.addChild(name).setBool(value);
}

int jsonRenameChild(Json json, string from, string to)
{
	Json child = json.getChild(from);
	if (child == NULL)
		return 0;
	child.setName(to);
	return 1;
