...
public:
	...
	virtual void render(const EngineWindowViewportPtr &window) { UNIGINE_UNUSED(window); }

	// The engine calls this function before each GUI render frame.
	virtual void gui(const EngineWindowViewportPtr &window) { UNIGINE_UNUSED(window); }
